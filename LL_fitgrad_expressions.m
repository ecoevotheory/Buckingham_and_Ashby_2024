...
syms SJ SA IJ IA tJ resA alpha tJm alpham rJ rA g gamma a0 f c1 c2 beta0 bJ bA tJ alphaJ q
...
Am=g*((bJ+g+alpha*(1-tJm)+gamma)*(bA+alpha*(1-tJm)+gamma)+f*(bJ+g+alpha*(1-tJm)+gamma)*lambdaAstar+f*lambdaJstar*(bA+lambdaAstar))+gamma*lambdaJstar;
Bm=(bJ+g+lambdaJstar)*((bA+alpha*(1-tJm)+gamma)*(bJ+g+alpha*(1-tJm)+gamma)*(bA+lambdaAstar)-gamma*lambdaAstar*(bJ+g+alpha*(1-tJm)+gamma))-gamma*((bA+lambdaAstar)*lambdaJstar*(bA+alpha*(1-tJm)+gamma)-gamma*lambdaJstar*lambdaAstar);
% Define the invasion fitness:
wH=((am*(1-q*(SJ+SA+IJ+IA))*Am)/Bm) -1;
wP=((beta0*(1-rJ)*SJ*(sqrt(alpham)*(bA+gamma+alpham*(1-tJ))+sqrt(alpham)*g)+beta0*(1-rA)*SA*sqrt(alpham)*(bJ+gamma+g+alpham*(1-tJ)))/((bJ+gamma+alpham*(1-tJ)+g)*(bA+gamma+alpham*(1-tJ))))-1;
...
% Determine the cross-derivatives at (tJ, alphaJ) and at points nearby:
