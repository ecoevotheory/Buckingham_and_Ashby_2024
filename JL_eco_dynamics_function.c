/***********************************************************************************************************
 * [t,SJ,SA,IJ,IA,EQFLAG] = JL_eco_dynamics_function(t_max,a0,g,q,c1a,c2a,rJ,rA,tA,tolJ_current,alpha_current,f,bJ,bA,beta0,gamma,eqtol,init_pop,strain_totalH,strain_totalP)
 ***********************************************************************************************************/

/* Compile in Matlab using mex JL_eco_dynamics_function.c */
 ...
 * Function prototypes
 *************************************/
int my_rungkut (double *T, double *SJ_out, double *SA_out, double *IJ_out, double *IA_out, double *EQFLAG, double *init_pop, double *tolJ, double *alpha, double *alphapower, struct PARAM *p);
void rkqs(double *SJ, double *SA, double *IJ, double *IA,  double *DSJDT, double *DSADT, double *DIJDT, double *DIADT,  double *h, double *hnext, double *SJ_SCALE, double *SA_SCALE, double *IJ_SCALE, double *IA_SCALE, double *tolJ,double *alpha, double *alphapower, struct PARAM *p);
void rkck(double *SJ, double *SA,  double *IJ, double *IA,  double *DSJDT, double *DSADT,  double *DIJDT, double *DIADT, double *SJout, double *SAout,  double *IJout, double *IAout, double *SJerr, double *SAerr,  double *IJerr, double *IAerr,  double h, double *tolJ, double *alpha, double *alphapower, struct PARAM *p);
void dynamic(double *SJ, double *SA, double *IJ, double *IA, double *DSJDT, double *DSADT, double *DIJDT, double *DIADT, double *tolJ, double *alpha, double *alphapower, struct PARAM *p);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    double *T, *SJ, *SA, *IJ, *IA, *EQFLAG, *init_pop, *tolJ, *alpha, *alphapower, *parameter;
   ...
   /* Allocate inputs */
    ...
    /* Call ODE solver */
    colLen = my_rungkut(t_temp, SJ_temp, SA_temp, IJ_temp, IA_temp,EQFLAG, init_pop, tolJ, alpha, alphapower, &p); 
...
int my_rungkut (double *T, double *SJ_out, double *SA_out, double *IJ_out, double *IA_out,  double *EQFLAG, double *init_pop, double *tolJ, double *alpha, double *alphapower, struct PARAM *p){
    ...
    do{
        ...
        dynamic(SJ, SA,  IJ, IA,  DSJDT, DSADT,  DIJDT, DIADT, tolJ, alpha, alphapower, p);
        ...
        rkqs(SJ, SA, IJ, IA, DSJDT, DSADT, DIJDT, DIADT, h, hnext, SJ_SCALE, SA_SCALE, IJ_SCALE, IA_SCALE, tolJ, alpha, alphapower, p); 
...
void rkqs(double *SJ, double *SA, double *IJ, double *IA,   double *DSJDT, double *DSADT,  double *DIJDT, double *DIADT,  double *h, double *hnext, double *SJ_SCALE, double *SA_SCALE, double *IJ_SCALE, double *IA_SCALE, double *tolJ, double *alpha, double *alphapower, struct PARAM *p)
{
    ...
    for(;;)
    {
        rkck(SJ, SA, IJ, IA, DSJDT, DSADT, DIJDT, DIADT, SJ_temp, SA_temp,  IJ_temp, IA_temp, SJ_err, SA_err,  IJ_err, IA_err,*h, tolJ, alpha, alphapower, p); 
...
void rkck(double *SJ, double *SA, double *IJ, double *IA, double *DSJDT, double *DSADT, double *DIJDT, double *DIADT, double *SJout, double *SAout, double *IJout, double *IAout, double *SJerr, double *SAerr, double *IJerr, double *IAerr, double h, double *tolJ, double *alpha, double *alphapower, struct PARAM *p){
    ...
    dynamic(SJtemp, SAtemp, IJtemp, IAtemp, SJk2, SAk2, IJk2, IAk2,  tolJ, alpha, alphapower, p);
    ...
    dynamic(SJtemp, SAtemp, IJtemp, IAtemp, SJk3, SAk3, IJk3, IAk3,  tolJ, alpha, alphapower, p);
    ...
    dynamic(SJtemp, SAtemp, IJtemp, IAtemp, SJk4, SAk4, IJk4, IAk4,  tolJ, alpha, alphapower, p);
    ...
    dynamic(SJtemp, SAtemp, IJtemp, IAtemp, SJk5, SAk5, IJk5, IAk5,  tolJ, alpha, alphapower, p); 
    ...
    dynamic(SJtemp, SAtemp, IJtemp, IAtemp, SJk6, SAk6, IJk6, IAk6,  tolJ, alpha, alphapower, p);
...
void dynamic(double *SJ, double *SA, double *IJ, double *IA,  double *DSJDT, double *DSADT, double *DIJDT, double *DIADT, double *tolJ, double *alpha, double *alphapower, struct PARAM *p){
    ...
    for(i=0;i<p->strain_totalH;i++){ 
	    ...
	    betaallinfecteds = betaallinfecteds + sqrt(alpha[j])*IJ[i+j*p->strain_totalH] + sqrt(alpha[j])*IA[i+j*p->strain_totalH];
    ...
    for(j=0;j<p->strain_totalP;j++){ 
        for (i=0; i<p->strain_totalH; i++){
	    totalparasites[j]=totalparasites[j]+sqrt(alpha[j])*IJ[i+j*p->strain_totalH]+sqrt(alpha[j])*IA[i+j*p->strain_totalH];
	}
    }
	...
	for (j=0; j<p->strain_totalP; j++){
	    ...
	    DIJDT[i+j*p->strain_totalH] = p->beta0*(1-p->rJ)*totalparasites[j]*SJ[i] -(p->g+p->gamma+p->bJ+alpha[j]*(1-tolJ[i]))*IJ[i+j*p->strain_totalH];
	    DIADT[i+j*p->strain_totalH] = p->g*IJ[i+j*p->strain_totalH]+p->beta0*(1-p->rA)*totalparasites[j]*SA[i]-(alpha[j]*(1-p->tA)+p->gamma+p->bA)*IA[i+j*p->strain_totalH];
	}
