function deriv=LL_find_parasite_singstrat_derivative(q,f,g,bJ,bA,beta0,rJ,rA,a0,c1,c2,gamma,tJval,initvec,orig_tmax)
...
for i=1:10000
    alpha=100*i/10000;
    [SJ,SA,~,~,~]=endemic_equilibrium_function(tJ,tJ,rJ,rA,g,a,q,beta0,bJ,bA,f,alpha,gamma,initvec,orig_tmax);
    parasite_fitgrad_sign(i)=0.5*(1-rJ)*SJ*(bA+alpha*(1-tJ)+g+gamma)*(bA+alpha*(1-tJ)+gamma)*(bJ+g+gamma-alpha*(1-tJ))+0.5*(1-rA)*SA*(bJ+g+gamma+alpha*(1-tJ))^2*(bA-alpha*(1-tJ)+gamma)-(1-tJ)*(1-rJ)*SJ*alpha*g*(bJ+g+alpha*(1-tJ)+gamma);
    if i>1
...
% There should only be one value for the derivative:
if length(deriv)>1
    deriv=[];
end
